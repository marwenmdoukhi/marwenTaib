...
@Component({
    selector: 'AbandonedActComponent',
    templateUrl: './consultAbandonedActComponent.html',
...
export class ConsultAbandonedActComponent implements OnInit, AfterViewInit {
    ...
    viewPdf(file: any) {
        ...
        pdfWindow.document.write("<iframe width='100%' height='100%' src='/documents/" + this.listDocument[0]['actId'] + '/' + file.name.replace(/\s/g, '') + ".pdf'></iframe>")
    }
        ...
        Promise.all([sendMailToAvocat]).then(results => {
            if (results[0] == 'email sent') {
                ...
                Promise.all([changeUsertoValidator]).then(results => {
                    ...
                    this.vm.displayAbandonedAct = false;
                });
            }
            else {
